...
from app.models import Profile, Profile_Genre, Genre, Musician, Venue, \
    Administrator, Media, Profile_Rate
...
def index():
    if current_user.is_authenticated:
        admin = Administrator.query.join(Profile).filter_by(
                profile_id=current_user.profile_id).first()
        relations = Profile_Genre.query.filter(
                    Profile_Genre.profile_id != current_user.profile_id).all()
        ...
        if account != 'venues':
            profiles = Profile.query.join(Musician).filter(
                       Musician.profile_id != current_user.profile_id,
                       Profile.block <= block_filter
                       ).with_entities(
                            Profile.username,
                            Profile.location,
                            Profile.profile_id,
            ...
            return render_template('home.html', profiles=profiles,
                                   relations=relations, genres=genres,
                                   admins=admin_users)
        else:
            profiles = Profile.query.join(Venue).filter(
                       Venue.profile_id != current_user.profile_id,
                       Profile.block <= block_filter
                       ).with_entities(
            ...
            media = Media.query.join(Venue).filter(
                    Venue.profile_id != current_user.profile_id).all()
            return render_template('venues.html', profiles=profiles,
                                   relations=relations, genres=genres,
    ...
    if request.method == 'POST':
        ...
        if term == "":
            ...
            if search_type == 'Artists':
                media = None
                if category == 'Name':
                    results = Profile.query.join(Musician).filter(
                    ...
                    results = Profile.query.join(Musician).filter(
                              Profile.location.contains(term),
                              Profile.block <= block_filter
                              ).with_entities(
                                    Profile.username,
                                    Profile.location,
                    ...
                    results = Profile.query.join(
                              Profile_Genre).join(Genre).join(Musician).filter(
                              Genre.genre_name.contains(term),
                              Profile.block <= block_filter
                              ).with_entities(
                                    Profile.username,
                                    Profile.location,
                ...
                media = Media.query.join(Venue).filter(
                        Venue.profile_id != current_user.profile_id).all()
                if category == 'Name':
                    results = Profile.query.join(Venue).filter(
                              Profile.username.contains(term),
                              Profile.block <= block_filter
                              ).with_entities(
                                    Venue.venue_capacity,
                    ...
                    results = Profile.query.join(
                              Profile_Genre).join(Genre).join(Venue).filter(
                              Genre.genre_name.contains(term),
                              Profile.block <= block_filter
                              ).with_entities(
                                    Venue.venue_capacity,
            ...
            try:
                ...
                return render_template('search_results.html', results=results,
                                       term=term, relations=relations,
                                       genres=genres, search_type=search_type,
                                       media=media)
            return render_template('search_results.html', results=results,
                                   term=term, relations=relations,
                                   genres=genres, search_type=search_type,
...
def block():
    ...
    admin = Administrator.query.join(Profile).filter(
            Administrator.profile_id == current_user.profile_id).first()
    if admin is not None:
        ...
        flash("You are not authorised to remove users from Musician's Network.\
...
def rate():
    ...
    if username == current_user.username:
        ...
        try:
            ...
            relation = Profile_Rate.query.filter_by(
                       target_id=target_user.profile_id,
                       profile_id=current_user.profile_id).first()
            # if current_user has never rated target_user
            if relation is None:
                rating = Profile_Rate(profile_id=current_user.profile_id,
                                      target_id=target_user.profile_id,
                                      rate=int(rate_value))
            ...
            ratings = Profile_Rate.query.filter_by(
                      target_id=target_user.profile_id).all()
            ...
            flash('Unable to rate {}. \
                  Please try again.'.format(target_user.username), 'error')
        return redirect(url_for('main.index', account='musicians'))
...
def blocked():
    ...
    if admin is not None:
        ...
        if account == 'musicians':
            media = None
            blocked_users = Profile.query.join(Musician).filter(
                            Musician.profile_id != current_user.profile_id,
                            Profile.block == 1
                            ).with_entities(
                                Profile.username,
            ...
            return render_template('search_results.html',
                                   results=blocked_users, term=term,
                                   relations=relations,
                                   genres=genres, search_type='Artists',
                                   media=media, admins=admin_users)
        elif account == 'venues':
            blocked_users = Profile.query.join(Venue).filter(
                            Venue.profile_id != current_user.profile_id,
                            Profile.block == 1
                            ).with_entities(
                                Venue.venue_capacity,
        ...
        flash("You are not authorised to view blocked users.")
        return redirect(url_for('main.index', account='musicians'))
...
def ratings():
    ...
    if account == 'musicians':
        ...
        results = Profile.query.join(Musician).filter(
                  Musician.profile_id != current_user.profile_id,
                  Profile.block == 0,
                  Profile.rating > lower,
                  Profile.rating <= upper
                  ).order_by(desc(Profile.rating)).with_entities(
                        Profile.username,
                        Profile.location,
                        Profile.profile_id,
                        Profile.rating,
        ...
        results = Profile.query.join(Venue).filter(
                  ...
                  Profile.rating <= upper
                  ).order_by(Profile.rating).with_entities(
                        Venue.venue_capacity,
                        Profile.username,
                        Profile.location,
                        Profile.rating,
