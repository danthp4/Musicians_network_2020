...
from wtforms import SubmitField, StringField, SelectField, TextAreaField, \
    SelectMultipleField, PasswordField, DateField, IntegerField
...
class ProfileForm(FlaskForm):
    ...
    location = SelectField(u'City',
                           choices=[('London', 'London'),
                                    ...
                                    ('Berlin', 'Berlin'),
                                    ('Cologne', 'Cologne'), ('NYC', 'NYC'),
                                    ('Tokyo', 'Tokyo'),
                                    ('Singapore', 'Singapore'),
                                    ('Sydney', 'Sydney')])
    genre = SelectMultipleField(label='Genre', choices=[('1', 'Rock'),
                                                        ('2', 'Drum and Bass'),
                                                        ('3', 'Techno'),
                                                        ('4', 'Funk'),
                                                        ('5', 'Disco'),
...
class MusicianForm(FlaskForm):
    gender = SelectField(u'Gender', choices=[('1', 'Male'), ('0', 'Female')],
                         validators=[DataRequired()])
    birthdate = DateField('Birthdate (YYYY/MM/DD)', format='%Y/%m/%d')
    availability = SelectField(u'Availability', choices=[('1', 'Available'),
                                                         ('0', 'Unavailable')],
                               validators=[DataRequired()])
...
class SettingsForm(FlaskForm):
    username = StringField('New Username', validators=[DataRequired(),
                                                       Length(max=20)])
    email = StringField('New Email address', validators=[DataRequired(),
                                                         Email()])
    ...
    def validate_username(self, username):
        ...
        if user is not None:
            raise ValidationError('Account is registered for that username.')
